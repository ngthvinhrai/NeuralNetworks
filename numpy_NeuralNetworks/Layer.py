...
import copy
import pickle
from NeuralNetworks.numpy_NeuralNetworks.Activation import Linear
...
class Dense(Layer):
    def __init__(self, output_shape:int, input_shape=0, activation=None, **kwargs):
    ...
    def backward(self, dL_A, optimizer, lr):
        ...
        if dL_A.ndim == 2:
            ...
            for t in reversed(range(self.input_shape[0] + 1 - k, self.input_shape[0] + 1)):
                ...
                dL_X += np.dot(dZo_X, dL_Zo) + np.dot(dZi_X, dL_Zi) + np.dot(dZc_X, dL_Zc) + np.dot(dZf_X, dL_Zf)
...
class Embedding(Layer):
    ...
    def built(self):
        if self.build == False:
            self.E = np.random.randn(self.input_shape, self.output_shape)
    ...
    def forward(self, X):
        if self.embedded == True:
            self.built()
            self.X = X
            self.A = self.activation(self.E[self.X])
        else:
    ...
    def backward(self, dL_A, optimizer, lr):
        if self.trainable:
            ...
            dL_E = np.zeros_like(self.E.T)
            for i, x in enumerate(self.X):
                dL_E[:, x] += dL_Z[:,i]

            self.grad_E, = optimizer(dL_E)
...
if __name__ == '__main__':
    ...
    X = np.random.randint(0, 500, 15)
    dlz = np.random.randn(300, 15)
    oh = np.eye(500)[X]
    dle = np.zeros((300,500))
    emb = Embedding(300, 500, embedded=True)
    print(emb.forward(X).shape)