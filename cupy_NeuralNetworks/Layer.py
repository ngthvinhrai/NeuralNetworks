...
class Layer:
    ...
    def backward(self, dL_A, optimizer, lr, gradient_norm=False): pass
...
class Dense(Layer):
    ...
    def backward(self, dL_A, optimizer, lr, gradient_norm=False):
        ...
        self.grad_W, self.grad_b = optimizer(dL_W, dL_b, norm=gradient_norm)
...
class RNN(Layer):
    ...
    def backward(self, dL_A, optimizer, lr, gradient_norm=False):
        ...
        if dL_A.ndim == 2:
            ...
            self.grad_U, self.grad_W, self.grad_bU = optimizer(dL_U, dL_W, dL_bU, norm=gradient_norm)
    ...
    def backward(self, dL_A, optimizer, lr, gradient_norm=False):
        ...
        if dL_A.ndim == 2:
            ...
            self.grad_Ui, self.grad_Wi, self.grad_bi, self.grad_Uo, self.grad_Wo, self.grad_bo, self.grad_Uc, self.grad_Wc, self.grad_bc = optimizer(
                dL_Ui, dL_Wi, dL_bi, dL_Uo, dL_Wo, dL_bo, dL_Uc, dL_Wc, dL_bc, norm=gradient_norm
            )
...
class ModernLSTM(LSTM):
    ...
    def built(self):
        if self.build == False:
            self.Ui = cp.random.randn(self.input_shape[1], self.output_shape)
            self.Wi = cp.random.randn(self.output_shape, self.output_shape)
            self.Uo = cp.random.randn(self.input_shape[1], self.output_shape)
            self.Wo = cp.random.randn(self.output_shape, self.output_shape)
            self.Uc = cp.random.randn(self.input_shape[1], self.output_shape)
            self.Wc = cp.random.randn(self.output_shape, self.output_shape)
            self.Uf = cp.random.randn(self.input_shape[1], self.output_shape)
            self.Wf = cp.random.randn(self.output_shape, self.output_shape)
    ...
    def backward(self, dL_A, optimizer, lr, gradient_norm=False):
        ...
        if dL_A.ndim == 2:
            ...
            for t in reversed(range(self.input_shape[0] + 1 - k, self.input_shape[0] + 1)):
                ...
                dL_X += cp.dot(dZo_X, dL_Zo) + cp.dot(dZi_X, dL_Zi) + cp.dot(dZc_X, dL_Zc) + cp.dot(dZf_X, dL_Zf)
            ...
            self.grad_Ui, self.grad_Wi, self.grad_bi, self.grad_Uo, self.grad_Wo, self.grad_bo, self.grad_Uc, self.grad_Wc, self.grad_bc, self.grad_Uf, self.grad_Wf, self.grad_bf = optimizer(
                dL_Ui, dL_Wi, dL_bi, dL_Uo, dL_Wo, dL_bo, dL_Uc, dL_Wc, dL_bc, dL_Uf, dL_Wf, dL_bf, norm=gradient_norm
            )
...
class Embedding(Layer):
    def __init__(self, output_shape, input_shape=0, activation=None, embedded=False, padding=True, max_length=None, trainable=False):
        super().__init__(output_shape, input_shape, activation)
        self.E = None
    ...
    def prepare(self, X):
        if self.max_length < 256: b = cp.int8
        elif self.max_length < 65536: b = cp.int16
        new_X = cp.zeros((len(X), self.max_length), dtype=b)

        if self.padding:
            for x, newx in zip(X, new_X):
                newx[(self.max_length-len(x)):] = cp.array([x], dtype=b)
            return cp.eye(self.input_shape, dtype=cp.int8)[new_X]
        
        else: return cp.eye(self.input_shape, dtype=cp.int8)[X]

    def forward(self, X):
        if self.embedded == True:
            self.built()
            self.X = self.prepare(X)
            self.A = self.activation(cp.matmul(self.X, self.E))
        else:
            self.A = self.prepare(X)
